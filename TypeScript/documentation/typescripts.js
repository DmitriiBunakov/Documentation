//*=============================================================================
//? /CLI//Установка/Install/
// https://www.typescriptlang.org/docs/handbook/compiler-options.html


//? /TSconfig.json/
// https://www.typescriptlang.org/tsconfig
//? на уровне где лежит данный файл, должен быть в корне проекта, это считается проектом который использует ts
...
//? Есть специальный тип ununique symbol, который запрещает даже сравнивать символы, т.к. всегда false они
/*
const s: unique symbol = Symbol();
const s2: unique symbol = Symbol();
console.log(s === s2);              ошибка, т.к. символы всегда разные
*/
...
//? /This/
//? в Ts this как первый аргумент функции стирается во время компиляции, это нужно для того, чтобы правильно устанавливать контекст через bind appply
/**
 * function test(this: Box, arg: string) {}
 * в
...
//? void используется, когда значение никак не будет использовано, это сильно отличается от того, что функция будет undefined.
/*
function test(): void {}
*/
...
//? 3) можно делать приватный сеттер публичный геттер, но не наоборот
/*
class Test {
...
//? useDefineForClassFields - настройка компилятора которая запрещает переопределять поле класса, если оно нам не нужно, те мы если хотим переопределить тип поля класса, то нужно declare написать, а не полностью переинициализировать поле класса
...
//? 2) protected из класса и подклассов, также после наследования можно расширить до публичного
...
//? Для типизации библиотеки, которую мы импортировали можно использовать declare слово. Мы объявляем эту переменную в глобальной области видимости мы можем на всем проекте обращаться к ней и видеть типизацию


//? Мы также можем типизировать модули в nodejs
/*
**
***
****
...
//*=============================================================================
//? /Iterators/
//? В es6 итераторы парсятся как обычно, а spread обьектов в object.assign, все что ниже, будет парсится как полифилы. Если хотим самые новые фичи, то нужно использовать EsNext
/*
**
***
****
...
//*=============================================================================
//? /Global/
//? можно модифицировать глобальные сущности, window, array.prototype и тд
/**
declare global {
    interface Array<T> {
        toObservable(): any;
...
//? noImplicitOverride нужно писать override, если хочешь переписать метод из родительского класса
/*
class Album {
    setup() {
    }
...
//? baseUrl отвечает за корневую точку входа в проект


//? в paths можно делать короткие алиасы для импортов
/*
"compilerOptions": {
    "baseUrl": "./",
    "paths": {
