...
//? /ErrorHandler/ - глобальный перехватчик ошибок, через него ангулар обрабатывает ошибки - можно переопределить
...
//? НЕ запускаются
//? async pipe - только помечается как грязный
...
//? 1) markForCheck - помечает явно компонент и все родительские, чтобы его проверить снова при следующем цикле, не запускает цикл самостоятельно(с 18 версии запускает)
...
//? внутренний класс, который запускает проверки, цикл запускается в requestAnimationFrame по дефолту, но бывает так, когда они запускаеют его в microTask
/*
**
***
****
...
//!=============================================================================
//? /Signals/
//? signal - метод который использоваться может вместо подписок и обычного состояния. Ангуляр везде где используются сигналы - будет их помечать как "зависимость данного компонента" и помечать markForCheck при изменении, но не запускать перерисовку(с 18 версии планирует запуск цикла обнаружения)
...
//?=============================================================================
//? /toSignal/
//? пописывается внутри на поток, то есть если мы сделаем этот вызов в сервисе - у нас будет вечный поток пока сервис не уничтожится
//? если поток завершился ошибкой, то при вызове сигнала будет синхронная ошибка и ее нужно обработать через try catch



//?=============================================================================
//? /toObservable/
//? из за того что сигналы асинхронные - поток будет тоже асинхронным и он будет эмитить данные только когда сигнал "стабилизуется", а тут значит будет 3 только в потоке созданном из сигнала
/*
...
//? можно в тэге указать аттрибут [select] в который можно передать css селектор любой, по которому будет проецироваться определенный контент
//? либо ngProjectAs для большей наглядности, но разницы нет
...
//? если мы проецируем контент в контент и в другой контент, нам везде нужно "получить и перепроецировать" на уровень ниже
/*
дочерний компонент
...
//? если ng-content не поместили -> можно написать в компоненте между тэгами дефолтный для отображения контент


//? контент который мы проецируем - он СОЗДАЕТСЯ В ТОМ КОМПОНЕНТЕ, ИЗ КОТОРОГО он проецируется, это значит, что если в дочернем компоненте мы проецируемый контент скрываем через if/else, то он удалится из верстки, но НЕ удалится как компонент, потому что он привязан к тому из которого идет проецирование. То есть memoryLeaks и тд нам обеспечены в теории
//? в такие моменты лучше всего делать через ngTemplateOutlet контент динамический, можно ниже заменить строковое получение ссылки #шаблона на директиву в contentChild директиву засовывать
/**
<app-child>
...
//? если инпут сеттер - тут могут быть баги, что сеттер завязан на других инпут параметрах, но они могут быть забинжены в неправильном порядке, и тогда сеттер будет вызван быстрее, чем забиндятся другие инпут параметры
...
//? Если компонент инжектирует директиву, например через useExisting, тогда порядок создания: HostDirective => Directives(инжектируемая директива) => Component
//? Если host директива инжектирует компонент: Component(инжектируемый) => HostDirective => Directive
//? 
//? Порядок выполнения хуков меняется соответственно, кроме ngAfterViewInit
...
//? можно не использовать - там минорные какие то штуки есть, для чего он нужен(если приложение кросспратформенное, и то это нужно было раньше - сейчас можно не использовать вообще)
//? главное что можно делать, это через di его переопределять и какие то свои штуки делать
...
//? Injector - создается на каждом компоненте/директиве и всегда уникальный
//? EnvironmentInjector - все глобальные модули объединенные в один имеют общий инжектор, каждый ленивый (модуль/компонент), имеет свой новый
...
//? /Host/
//? это ограничитель поиска - последнее место где может найти сущность - это VIEW родителя в котором создан данный компонент/директива(viewProviders), то есть если у нас директива в компоненте создается, то последнее место где она найдет токен - это viewProviders компонента в котором она создается
//? если компонент дочерний внедряет токен, то viewProviders последнее место родителя где токен можно будет найти, даже если providers есть - до него уже не дойдет
//? если и дочерний и родительский viewProviders имеют один и тот же токен, то дочерний найдет у себя его, но если бы его не было у ребенка - последнее место где мог бы найти - родительский viewProviders, чтобы начать поиск выше в данном примере - skipSelf + host
...
//? 4) initialNavigation - 
//? 5) errorHandler - 
//? 7) onSameUrlNavigation - ignore - не запускает ивенты/проверки, reload - запускает проверки
...
//? через outlet настроку роута можно определенный путь в определенный outlet рендерить
...
//? setValue принимает обьект настроек, можно сделать так, чтобы не вызывались status/valueChanges, также чтобы родительская группа не узнала о том, что значение изменилось, а также можно сделать, чтобы ngValueAccessor не вызывались хуки установки



//? /FormControlName/
//? Привязывает по имени к controlContainer родительскому
...
//? addControl/setControl - раздизейбливают форму, они запускают перерасчет формы по дефолту, можно отключить emitEvent
//? register/remove - не раздизейбливают форму
//? remove - не запускает обновление данных в форме
//? register - не запускает
...
//? formControl/formControlName/NgModel директивы которые создают провайдера NgControl, этот провайдера предоставляет эти самые директивы, директивы выступают в виде управления контролом
//? formArrayName/formGroup/formGroupName/ngModelGroup создают controlContainer провайдер



//? NgForm похож на FormGroup, только используется для самой верхней формы
...
//!================================================================================================================================================
//? /Интернационализация/
//? DEFAULT_LOCALE_ID/LOCALE_ID токены
