...
//? eсли onPush, то цикл будет запущен только при DOM событии в этом компоненте или дочернем, изменении input параметров, а также при явной пометке на изменения
//? eсли дочерний компонент имеет default стратегию, то он будет перерисован, и вызовет detectChanges вниз по дереву, и так далее
//? при встрече onPush стратегии, если цикл обнаружений был запущен снизу, до текущего компонента проверяемого, либо с него, то он будет проверен и перерисован, все компоненты ниже триггера, не будут перерисованы и проверены
//? при вызове detectChanges происходит запуск цикла с данного компонента и вниз по дереву, компоненты ниже имеющие onPush не будут перерисованы, default - перерисуются
//? при вызове markForCheck - компонент будет помечен, что его и всех родителей нужно проверить при следующем цикле, сам данный метод не запускает цикл обнаружения изменений. Работает, даже если родители имеют onPush стратегию
//? если в компоненте используется async pipe, то при изменении подписки, компонент будет также помечен, что его нужно проверить
//? если компонент отсоединен от проверок, то единственное, что поможет, это вызов в нем detectChanges



//? Default - запускает цикл при любой асинхронной операции, изменении значения переменной, событиях


//? OnPush - при изменении input, обработчиках в компоненте и ниже по дереву, а также при явном вызове обнаружения изменений, detectChanges, markForCheck, ApplicationRef.tick
...
//? /providedIn/
//? any - значение, все сервисы в ленивых модулях получат уникальный экземпляр класса, в обычных модулях будет синглтон
...
//? главный модуль импортирует модуль, в котором они все объявлены
//? root или platform значение для providedIn


//? Они будут созданы заново, если:
//? в модуле commonModule(общий кастомный модуль) мы объявим сервис, и этот модуль будем включать в каждый другой модуль, там будут созданы новые экземпляры
//? в компоненте в массив providers укажем сервис для инжектирования
//? в ленивых модулях, если в модуль импортируется модуль, в котором уже есть этот сервис или мы просто объявляем сервис, который уже выше в иерархии объявлен
...
//? Чтобы в приложении использовать html5 стратегию навигации, тоже самое, что и путь файла на сервере, в index.html приложения нужно добавить <base href="/"> тэг сразу в head тэге, либо можно APP_BASE_HREF инжектировать в приложение
...
//? перехватчики работают только на верхнем уровне приложения, главный модуль
