...
//? <moduleName> - если сделаем так, то сервис будет удален из сборки, если он нигде не используется
...
//? главный модуль импортирует модуль, в котором они все объявлены, а также если они объявлены в нескольких модулях, которые в свою очередь в конечном итоге в главный импортированы
//? если в ленивом модуле объявлен сервис, но этот модуль целиком не загружается лениво, и отдельный компоннт уже используется в сборке и так когда в путях прописываем component: <SomeComponent>
//? root или platform значение для providedIn


//? Они будут созданы заново, если:
//? в компоненте в массив providers, viewProviders укажем сервис для инжектирования
