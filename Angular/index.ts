...
//? например contentChild(Token) - где Token - это абстрактный класс, какая то легкая сущность, по которой будет получен di сущность, и нам не обязательно тянуть что то большое по этому токену в сборку
/*
**
***
****
...
//? если компонент onPush - то он будет перерисовываться только при изменении инпут параметров, на обробатчики событий в самом компоненте(потому что все такие функции при их вызове оборачиваются ангуляром в свою функцию которая помечает этот компонент и всех предков как dirty), либо на обработчики событий запущенные из дочерних компонентов. Обработчики событий из дочерних компонентов запускают и почему то заставляют проверить все компоненты с onpush стратегией вниз по дереву до компонента, где этот обработчик был вызван. Также если компонент является onPush цикл обнаружений не зайдет в него, если только его не пометить markForCheck, detectChanges не вызвать в нем, или же если же обработчик был вызван в нем либо ниже по дереву.
...
//? /Host/
//? поиск будет производиться в данном инжекторе
//? если вместе со skipSelf - то поиск будет производиться в том инжекторе, куда присоединен данный компонент или директива.
//? используем компонент в компоненте - будет искать во viewProviders
//? использует директиву в компоненте - будет искать во viewProviders
//? директива в директиве - ищет у родительской директивы - потому что "host" для дочерней - родительская, если и там не найдет - пойдет во viewProviders компонента в котором присоединена
//? viewProviders - последнее место где может быть что то найдено
...
//? 7) canActivate - можно ли активировать КОМПОНЕНТ
//? 8) canActivateChild - можно ли активировать дочерние КОМПОНЕНТЫ
//? 9) canDeactivate - можно ли уйти со страницы
//? 10) canMatch - можно ли загрузить и в целом сопоставить данный роут
