...
//? -> http запрос, тут же отправляется OPTIONS запрос, чтобы получить что поддерживает сервер (безопасный запрос, он не меняет ничего)
//? -> dns обработка
//? -> браузер делает запрос на сервер, есть ли ssl
...
//? по умолчанию если нет SameSite, то он SameSite=Lax, или если SameSite=None, то должны быть куки переданы по Secure(иначе будет ошибка)
...
//? также может подменять DNS и отправлять пользователей на вредоносные сайты
...
//? * font-display - разные стратегии рендеринга текста
...
//? несколько файлов могут обращаться к одному и тому же воркеру, если они работают в одном порте
...
//? нельзя использовать фукнции, domNodes - ошибка
