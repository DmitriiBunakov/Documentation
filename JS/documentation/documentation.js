...
//? 1) src - путь к скрипту, если указан, то код внутри тэга будет проигнорирован
//? 2) charset - указывает кодировку файла, используется вместе в src
//? 3) async - получение скрипта происходит асинхронно, а выполнение будет произведено сразу после скачивания, очередность выполнения не гарантируется, также может прервать дальнейшее построение DOM, если загружен раньше чем он построен - для самых приоритетных модулей, до выполнения события load на документе
//? 4) defer - получение происходит асинхронно, но выполнение будет просиходить только после того, как страница будет загружена целиком, что то не очень важное, порядок соблюдаться будет, до DOMContentLoaded
...
//? В примере ниже, если геттер не переопределить на уровне наследуемого класса, но переопределить сеттер, то геттер в прототип будет записан как undefined, а сеттер как обычный сеттер. Странноватое поведение.
/*
class Base {
    #data;
    get data() {
...
//? обьект настроек это: capture - boolean(отвечает за то, когда будет поймано событие, на погружении - true, на всплытие - false по дефолту),        once - boolean(обработчик будет вызван один раз и удален),        passive - boolean(обработчик никогда не может быть отменен через preventDefault, некоторые события по дефолту идут уже с true, поэтому может не работать preventDefault)
...
//? Микрозадачи - колбэки then, catch, finally у промисов, а также же колбэки mutation observer, в nodejs process.nextTick тоже находится в микрозадачах, и имеет высший приоритет среди микрозадач
...
//? nodejs
//? Все синхронные задачи (это один тик - исполнение задач синхронных)
//? nextTick колбэки - как только все синхронные задачи выполнены (то есть зарегестрированные колбэки любые когда попадают в стэк вызовов)
//? microtasks (queueMicrotask засовывает зарезолвленный промис прям в конец готовых микрозадач, и будет исполнен в этом же цикле)
//? nextTick
//? timers
//? nextTick
//? immediate //TODO выполняются иногда раньше таймеров
//? nextTick
/*
**
***
...
//? * динамический контент делать так, чтобы он по максимому не смещал контент
...
//? * тоже самое с js - в head - блокирует и тот который без async/defer аттрибутов
...
//? * использовать анимацию канвас вместо свг, чтобы не блокировать движок, также можно на видеокарту кинуть css какой то, и не вызывать reflow/repaint через код
...
//? * шрифты встраивать сразу на страницу либо если со стороннего ресура - то предзагружать, чтобы не было скачков текста и тд, и явно сразу указывать что использует шрифт, потому что пока не будет использования шрифта - он загружен не будет
//? font-display
...
//? * picture/source - адаптивно загружать нужные изображения под устройство
...
//!================================================================================================================================================
//? /Localstorage/Sessionstotage/
// https://learn.javascript.ru/localstorage

//? Существуют для хранения данных, к ним не может получить доступ бэкенд, хорошая замена куки, т.к. данные не отправляются на бек. Оба хранилища используют только строковые данные

//? Localstorage сохраняет данные даже после закрытия браузера

//? Sessionstotage сохранит данные только данной вкладки пока ее не закрыть, перезагрузка не сбрасывает данные, только закрытие. Если же открыть тот же сайт в другой вкладке, для нее будет создан свой sessionstotage
...
//? События:
//? 1) success - база успешно открыта
//? 2) upgradeneeded - локальная версия базы отличается от той, которая сейчас есть на сайте, а также если ее просто не существует пока что, тут можно проинициализировать
//? 3) error - когда локальная версия выше текущей загружаемой
//? 4) blocked - 


//? Базы данных можно удалять, создавать, получить все
